...
public function Listar(){
	try{
		$result = array();
		$stm = $this->pdo->Prepare('SELECT customer.* FROM customer, customer_has_role where customer.documentNumber = customer_has_role.documentNumber and customer_has_role.roleId = 4');
		$stm->execute();
...
///--------- SUBIR FOTO ----
		 ...
		 $sql="UPDATE customer SET firstName= ?, secondName= ?, firstLastName= ?, secondLastName= ? WHERE documentName=? and documentNumber=? ";
	...
	$sql="UPDATE users SET passwordUser= ?, photo= ?, mail= ?  WHERE documentName=? and documentNumber=?";
	$this->pdo->prepare($sql)->execute(
		array(
